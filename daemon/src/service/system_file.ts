...
export default class FileManager {
  ...
  private matchRule(
    ...
    if (pattern.endsWith("/")) {
      ...
      if (allowParentAccess) {
        const normalizedRule = dirPattern.startsWith("/") ? dirPattern.slice(1) : dirPattern;
        return normalizedPath.length > 0 && normalizedRule.startsWith(`${normalizedPath}/`);
      }
  ...
  private evaluateIgnoreRules(normalizedPath: string, rules: string[], minimatchLib: any): boolean {
